...
@csrf_exempt
def get_update_or_delete_user(request: HttpRequest, user_id: int) -> HttpResponse:
...
def add_to_cart(request: HttpRequest, product_id: int) -> HttpResponse:
    ...
    response = HttpResponseRedirect(reverse('app:get_products'))
...
def remove_from_cart(request: HttpRequest, product_id: int) -> HttpResponse:
    ...
    if cart:
        ...
        if str(product_id) in cart:
            ...
            response = HttpResponseRedirect(reverse('app:get_cart'))
            response.set_cookie('cart', json.dumps(cart))
            return response
    return HttpResponse(status=204)
